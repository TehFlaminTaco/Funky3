...
public class StringLiteral : Expression
{
    ...
    {
        stackName = $"/* String: {Value.Replace('/', '_')} */ VarNewConstString({Value.Replace("\r", "\\r").Replace("\n", "\\n").Replace("\t", "\\t")})";
        return "";
    }
}
...
public class InterpolatedStringLiteral : Expression
{
    ...
    public override string GenerateInline(StreamWriter header, out string stackName)
    {
        ...
        {
            stackName = $"/* String: `{Strings[0].Replace('/', '_')}` */ VarNewConstString({ToDoubleQoute(Strings[0])})";
            return "";
        }
