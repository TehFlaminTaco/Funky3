...
public abstract class ListEntry
{
    ...
    {
        // Try and get an EntryUnpack
        var unpack = EntryUnpack.TryParse(tokens, index);
        if (unpack.entryUnpack != null)
        {
            return unpack;
        }
...
public class EntryUnpack : ListEntry
{
    public string[] Names { get; set; }
    public Expression Value { get; set; }

    public EntryUnpack(string[] names, Expression value)
    {
        this.Names = names;
        this.Value = value;
    }

    public static new(EntryUnpack? entryUnpack, int index) TryParse(List<Token> tokens, int index)
    {
        int i = index;
        // Get a left bracket
        if (tokens[i].Type != TokenType.Punctuation || tokens[i].Value != "(") return (null, index);
        Parser.RegisterFurthest(++i);
        List<string> names = new();
        while (true)
        {
            while (tokens[i].Type == TokenType.Punctuation && tokens[i].Value == ",") i++;
            Parser.RegisterFurthest(i);
            if (tokens[i].Type == TokenType.Punctuation && tokens[i].Value == ")")
            {
                i++;
        ...
        i = value.index;
        return (new EntryUnpack(names.ToArray(), value.expression), i);
    }
}
