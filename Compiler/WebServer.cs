    ...
    public static void Main(string[] args)
    {
        ...
        while (true)
        {
            try
            {
                ...
                if (url.LocalPath == "/getExample")
                { // Load an example into the user's session
                ...
                if (url!.LocalPath == "/funky3.wasm")
                {
                    if (!UserCodes.ContainsKey(session))
                    {
                        ...
                        response.Close();
                        Console.WriteLine("Sent hello world due to no session.");
                        continue;
                    }
                    ...
                    if (!CompiledWasm.ContainsKey(session))
                    {
                        ...
                        response.OutputStream.Write(CompiledWasm["helloworld.fnk"].compiled);
                        response.Close();
                        Console.WriteLine("Sent hello world due to no compiled wasm.");
                        continue;
                    }
